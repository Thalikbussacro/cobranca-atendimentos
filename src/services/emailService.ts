...
function gerarHTMLCobranca(cobranca: Cobranca): string {
...
function gerarTextoPlanoCobranca(cobranca: Cobranca): string {
  const valorTotal = calcularValorTotal(cobranca)
  const precoHoraFormatado = formatarPrecoHora(cobranca.precoHora)
...
export async function enviarCobrancaEmail(cobranca: Cobranca): Promise<void> {
