...
import { gerarPdfCobranca } from "../pdf/relatorioAtendimento.js";
...
function gerarTextoPlanoCobranca(cobranca) {
  ...
  texto += `DETALHAMENTO DOS ATENDIMENTOS PODE SER VISTO NO PDF EM ANEXO\n`;
...
export async function enviarCobrancaEmail(cobranca) {
  ...
  const pdfBuffer = await gerarPdfCobranca(cobranca);
  const nomeArquivo = `atendimentos_${cobranca.cliente.replace(/[^a-zA-Z0-9]/g, "_")}_${cobranca.periodo.replace(/[^a-zA-Z0-9]/g, "_")}.pdf`;
